    ...
    override suspend fun onAuthenticatedRequest(call: ApplicationCall, token: AuthorizationToken) {
        withContext(Dispatchers.IO) {
            ...
            val alreadyStoredImage = m.transaction {
                StoredImages.slice(StoredImages.id).select {
                    StoredImages.originalShaHash eq originalChecksum
                }.firstOrNull()
            }

            if (alreadyStoredImage != null) {
                call.respondJson(UploadImageResponse(alreadyStoredImage[StoredImages.id].value))
                return@withContext
            }
            ...
            if (!attributes.skipOptimizations) {
                ...
                checksum = m.fileUtils.calculateChecksum(fileToBeStored)
                ...
                if (alreadyStoredOptimizedImage != null) {
                    call.respondJson(UploadImageResponse(alreadyStoredOptimizedImage[StoredImages.id].value))
                    return@withContext
                }
            }
            ...
            call.respondJson(UploadImageResponse(storedImage.id.value))
        }
    }
}