...
class GetImageLinkInfoRoute(m: DreamStorageService) : RequiresAPIAuthenticationRoute(m, "/api/v1/images/links/{folderWithFile...}") {
    companion object {
    ...
    override suspend fun onAuthenticatedRequest(call: ApplicationCall, token: AuthorizationToken) {
        ...
        val imageLink = m.transaction {
            ImageLink.find {
                ImageLinks.createdBy eq token.id and (ImageLinks.folder eq folder and (ImageLinks.file eq file))
            }.firstOrNull()
        } ?: run {
